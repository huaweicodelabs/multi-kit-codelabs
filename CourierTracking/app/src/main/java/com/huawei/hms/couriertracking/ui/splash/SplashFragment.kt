...
import com.huawei.hms.couriertracking.databinding.FragmentSplashBinding
...
class SplashFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        splashViewModel.initializeCloudDB()
        collectUIState()
    }
    ...
    private fun collectUIState() {
        lifecycleScope.launch {
            splashViewModel.uiState.collectLatest { uiState ->
                when(uiState){
                    Success -> { findNavController().navigate(R.id.action_splashFragment_to_homeFragment) }
                    Loading -> { binding.progressBarSplash.visibility = View.VISIBLE }
                    is Error -> {
