...
public class LocalTextTransactor extends BaseTransactor<MLText> {
    ...
    private String getLanguage() {
        String position = MySharedPreferences.getInstance(mContext).getStringValue(Constant.POSITION_KEY);
    ...
    public void stop() {
        try {
            ...
            Log.e(LocalTextTransactor.TAG, "Exception thrown while trying to close text transactor: " + e.getMessage());
        }
    }
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            for (int j = 0; j < lines.size(); j++) {
                ...
                if (lines.get(j).getStringValue() != null && lines.get(j).getStringValue().trim().length() != 0) {
                    BaseGraphic textGraphic = new LocalTextGraphic(graphicOverlay, lines.get(j));
